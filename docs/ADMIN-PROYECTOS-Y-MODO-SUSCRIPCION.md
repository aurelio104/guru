...
## 2. Fuente de la lista: la misma que APlat
...
No hace falta una “lista de admin” distinta: la lista es la de “proyectos en producción” que ya usa APlat. El admin solo añade **por proyecto** el estado “modo suscripción” y los datos de pago.
...
- **Opción A – En la API de APlat (Koyeb)**  
...
- **Opción C – Híbrido**  
  Lista de proyectos = `portfolio-production-urls.json` (o sync Vercel). Estado “modo suscripción” y datos de pago = API APlat (tabla en SQLite o similar). El panel de admin llama a la API para leer/escribir ese estado; la lista de “todos los desplegados” se construye leyendo el JSON (o un endpoint que devuelva eso).
...
3. Esa lista es la que usa el **portafolio** de APlat y la que debe usar el **admin**.
...
Así, “que automáticamente pase a ser administrable” se cumple: **en cuanto el proyecto está en la lista de desplegados** (por sync o manual), aparece en el admin; el “automático” es que no hace falta crear a mano el proyecto en otro sitio, solo entra en la misma lista que ya alimenta APlat.
...
Con esto quedan cubiertos: (1) todos los proyectos desplegados como en APlat, (2) botón que activa modo suscripción y datos de pago, (3) los que no lo tengan = normales, (4) lo que despliegues mañana pasa a ser administrable en cuanto entre en esa lista.
