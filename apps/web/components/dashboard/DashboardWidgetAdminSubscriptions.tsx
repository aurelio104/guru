    ...
    >
      <div className="flex items-center justify-between gap-2 mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-aplat-text">Estado de suscripciones</h2>
        </div>
        <div className="flex items-center gap-2">
          <button
            type="button"
          ...
          <button type="button" onClick={fetchSubs} disabled={loading} className="p-1.5 rounded-lg text-aplat-muted hover:text-aplat-text">
            Actualizar
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-aplat-muted text-sm py-4">No hay suscripciones. Envía invitaciones desde el widget Suscripciones.</p>
      ) : (
        <ul className="space-y-2 max-h-[320px] overflow-y-auto">
          {list.map((s) => (
            ...
            >
              ...
              <span className="text-aplat-muted">+{s.phone}</span>
              <span
              ...
              {s.amount != null && <span className="text-aplat-emerald">${s.amount}</span>}
              <button
                type="button"
                onClick={() => handleMarkPaid(s)}
                disabled={processing === s.id}
                className="ml-auto inline-flex items-center gap-1 rounded-lg bg-aplat-emerald/20 hover:bg-aplat-emerald/30 text-aplat-emerald border border-aplat-emerald/30 px-2 py-1 text-xs font-medium disabled:opacity-60"
              >
