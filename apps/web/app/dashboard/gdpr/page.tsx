      ...
      {!loading && items.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            ...
            <p className="text-xl font-bold text-aplat-text">{items.length}</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
          ...
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <p className="text-aplat-muted text-xs uppercase tracking-wider">En progreso</p>
            <p className="text-xl font-bold text-aplat-cyan">{inProgress}</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <p className="text-aplat-muted text-xs uppercase tracking-wider">Hecho</p>
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="space-y-6">
          {orderedCategories.length ? orderedCategories.map((cat) => (
                      ...
                      {editingNotesId === item.id ? (
                        <div className="mt-2">
                          <input
                            ...
                            className="w-full rounded-lg bg-white/5 border border-white/10 px-2 py-1 text-sm text-aplat-text placeholder:text-aplat-muted"
                            autoFocus
                          />
                        </div>
                      ) : (
                        <button
                          ...
                          className="text-xs text-aplat-muted hover:text-aplat-cyan mt-1.5"
                        >
            ...
            <p className="text-aplat-muted py-6 text-center">No hay ítems en el checklist.</p>
          )}
        </div>
      )}
    </div>
