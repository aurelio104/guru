...
const ORDER_STATUS_COLOR: Record<string, string> = {
  ...
  processing: "bg-aplat-cyan/20 text-aplat-cyan",
  shipped: "bg-aplat-violet/20 text-aplat-violet",
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-aplat-muted text-sm">Productos y pedidos</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {isMaster && (
      ...
      {!loading && (products.length > 0 || orders.length > 0) && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <p className="text-aplat-muted text-xs uppercase tracking-wider">Productos</p>
            <p className="text-xl font-bold text-aplat-text">{products.length}</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <p className="text-aplat-muted text-xs uppercase tracking-wider">Pedidos</p>
            <p className="text-xl font-bold text-aplat-text">{orders.length}</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <p className="text-aplat-muted text-xs uppercase tracking-wider">Pendientes</p>
            <p className="text-xl font-bold text-amber-400">{pendingOrders}</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <p className="text-aplat-muted text-xs uppercase tracking-wider">Activos</p>
      ...
      {isMaster && showProductForm && (
        ...
        >
          <h2 className="text-lg font-semibold text-aplat-text mb-3">Nuevo producto</h2>
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              <label className="block text-sm font-medium text-aplat-muted mb-1">Precio *</label>
              <input
                type="number"
                step="0.01"
                min="0"
            ...
            <div>
              <label className="block text-sm font-medium text-aplat-muted mb-1">Moneda</label>
              <select
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <section>
            ...
            {products.length === 0 ? (
              ...
              >
                <ShoppingCart className="w-10 h-10 text-aplat-muted/70 mx-auto mb-3" />
                <p className="text-aplat-text font-medium mb-1">No hay productos</p>
                ...
                {products.map((p) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      {p.description && (
                        <p className="text-sm text-aplat-muted mt-0.5 line-clamp-1">{p.description}</p>
                      )}
          ...
          <section>
            ...
            {orders.length === 0 ? (
              ...
              >
                <ShoppingCart className="w-10 h-10 text-aplat-muted/70 mx-auto mb-3" />
                <p className="text-aplat-text font-medium mb-1">No hay pedidos</p>
                <p className="text-aplat-muted text-sm max-w-sm mx-auto">
              ...
              <ul className="space-y-2 max-h-[400px] overflow-y-auto">
                {orders.slice(0, 20).map((o) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-aplat-text text-sm">{o.id}</p>
                      <p className="text-aplat-muted text-sm">
                        {o.total} {o.currency}
                        {o.customerName && ` · ${o.customerName}`}
                        {o.customerEmail && !o.customerName && ` · ${o.customerEmail}`}
                      </p>
                      <p className="text-xs text-aplat-muted mt-1">{new Date(o.createdAt).toLocaleString("es")}</p>
                    </div>
                    {isMaster ? (
                      <select
                        value={o.status}
                        onChange={(e) => updateOrderStatus(o.id, e.target.value)}
                        className={`rounded-lg px-2 py-1 text-xs shrink-0 ${ORDER_STATUS_COLOR[o.status] ?? ""} bg-white/5 border border-white/10 text-aplat-text`}
                      >
                      ...
                      <span className={`inline-block px-2 py-0.5 rounded text-xs shrink-0 ${ORDER_STATUS_COLOR[o.status] ?? "bg-white/10 text-aplat-muted"}`}>
