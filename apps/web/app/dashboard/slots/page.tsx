    ...
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-guru-text">Slots / Reservas</h1>
        </div>
        <button type="button" onClick={fetchData} className="p-2 rounded-xl border border-white/20 hover:bg-white/5 text-guru-muted hover:text-guru-text">
      ...
      {loading && resources.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <section>
            <h2 className="text-lg font-semibold text-guru-text mb-3">Recursos</h2>
            {resources.length === 0 ? (
              <p className="text-guru-muted py-4">No hay recursos. Crear desde API (rol master).</p>
            ) : (
              <ul className="space-y-2">
                {resources.map((r) => (
                  <li key={r.id} className="rounded-xl border border-white/10 bg-white/5 p-3">
                    <p className="font-medium text-guru-text">{r.name}</p>
                    <p className="text-sm text-guru-muted">{r.slotDurationMinutes} min/slot</p>
                  </li>
                ))}
              </ul>
            )}
          ...
          <section>
            <h2 className="text-lg font-semibold text-guru-text mb-3">Últimas reservas</h2>
            {bookings.length === 0 ? (
              <p className="text-guru-muted py-4">No hay reservas.</p>
            ) : (
              <ul className="space-y-2">
                {bookings.map((b) => (
                  <li key={b.id} className="rounded-xl border border-white/10 bg-white/5 p-3">
                    <p className="text-guru-text text-sm">{b.title || "Reserva"}</p>
                    <p className="text-xs text-guru-muted">
